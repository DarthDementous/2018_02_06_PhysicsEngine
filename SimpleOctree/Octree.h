...
namespace brandonpelfrey {
	...
	class Octree {
		...
		// SEB CUSTOM MODIFICATION: Ensure data is deleted as well as children
		~Octree() {
			// Delete memory held by data
			delete data;
		...
		// SEB CUSTOM MODIFICATIONS: GLM Library Getters
		glm::vec3 GetOrigin() {
			return glm::vec3(origin.x, origin.y, origin.z);
		}

		glm::vec3 GetHalfExtents() {
			return glm::vec3(halfDimension.x, halfDimension.y, halfDimension.z);
		}

		// SEB CUSTOM MODIFICATION: Clear tree
		void Clear() {
			// Recursively destroy octants
			for (int i = 0; i < 8; ++i) {
				delete children[i];
		...
		// SEB CUSTOM MODIFICATION: Recursively retrieve all nodes (octrees in themselves) currently in octree
		void GetAllNodes(std::vector<Octree*>& a_nodes) {
			// Current octree has no children
			if (isLeafNode()) {
				// Add to nodes and return
				a_nodes.push_back(this);

				return;
			}

			// Current octree has children
			else {
				// Add to nodes
				a_nodes.push_back(this);

				// Loop through all eight children and retrieve nodes from child octree
				for (int i = 0; i < 8; ++i) {

					children[0]->GetAllNodes(a_nodes);
				}
			}
		}
